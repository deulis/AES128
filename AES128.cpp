...
void AES128::Encrypt(unsigned char* message, unsigned char* encryptedMessage, int blocks)
{
	for (int i = 0; i < blocks; i++) {
		EncryptBlock(message + (i * 16), encryptedMessage + (i * 16));
	}
}

void AES128::Decrypt(unsigned char* encryptedMessage, unsigned char* message, int blocks)
{
	for (int i = 0; i < blocks; i++) {
		DecryptBlock(encryptedMessage + (i * 16), message + (i * 16));
	}
}
 ...
 */
void AES128::SubBytesEncryption(unsigned char* state) {
	for (int i = 0; i < 16; i++) {
		state[i] = s[state[i]];
	}
}
...
void AES128::ShiftRowsEncryption(unsigned char* state) {
 ...
 */
void AES128::RoundEncrytion(unsigned char* state, unsigned char* key) {
	SubBytesEncryption(state);
	ShiftRowsEncryption(state);
	MixColumns(state);
 ...
 */
void AES128::EncryptBlock(unsigned char* message, unsigned char* encryptedMessage) {
	...
	for (int i = 0; i < numberOfRounds; i++) {
		RoundEncrytion(state, expandedKeys + (16 * (i + 1)));
	}

	FinalRound(state, expandedKeys + 160);
...
void AES128::ShiftRowsDescrytion(unsigned char* state) {
	unsigned char tmp[16];
 ...
 */
void AES128::SubBytesDescrytion(unsigned char* state) {
 ...
 */
void AES128::RoundDescription(unsigned char* state, unsigned char* key) {
	SubRoundKey(state, key);
	InverseMixColumns(state);
	ShiftRowsDescrytion(state);
	SubBytesDescrytion(state);
}
 ...
 */
void AES128::DecryptBlock(unsigned char* encryptedMessage, unsigned char* decryptedMessage)
{
	...
	InitialRound(state, expandedKeys + 160);

	int numberOfRounds = 9;

	for (int i = 8; i >= 0; i--) {
		RoundDescription(state, expandedKeys + (16 * (i + 1)));
	}

	SubRoundKey(state, expandedKeys); // Final round
...
void AES128::KeyExpansionCore(unsigned char* in, unsigned char i) {
 ...
 */
void AES128::SetKey(unsigned char inputKey[16]) {
...
const unsigned char AES128::s[256] =
{
...
const unsigned char AES128::mul2[] =
{
...
const unsigned char AES128::mul3[] =
{
...
const unsigned char AES128::rcon[256] = {
...
const unsigned char AES128::inv_s[256] =
{
...
const unsigned char AES128::mul9[256] =
{
...
const unsigned char AES128::mul11[256] =
{
...
const unsigned char AES128::mul13[256] =
{
...
const unsigned char AES128::mul14[256] =
{
